...
if uploaded_file is not None:
    ...
    st.sidebar.text(f"Inicio del lote: {start_date} {time_labels[start_time_index]} hrs")
    st.sidebar.text(f"Fin del lote: {end_date} {time_labels[end_time_index]} hrs")
    ...
    out_of_work_data = data[~data.index.isin(filtered_data.index)]

    # Mostrar advertencias si hay eventos fuera del horario de trabajo
    if not out_of_work_data.empty:
        st.warning(f"⚠️ Eventos ocurridos fuera del horario de trabajo ({start_date} {time_labels[start_time_index]} - {end_date} {time_labels[end_time_index]}):")
